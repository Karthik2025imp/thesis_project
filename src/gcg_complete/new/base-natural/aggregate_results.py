"""
aggregate_results.py -- combine per-process summary CSVs and plot loss curves.

Run after all 5 prefix_run_{0..4}.py processes have completed:
    cd ~/project
    python3 src/aggregate_results.py

Reads:  insertion_accelerated_refinement_summary_p{0..4}.csv
Writes: insertion_accelerated_refinement_summary_all.csv  (combined)
        insertion_accelerated_refinement_loss_curves.png  (per-prompt trace)
        insertion_accelerated_refinement_aggregate.txt    (printed stats)

Folder-local: this copy lives in src/gcg_complete/new/base-natural/ and
reads/writes exclusively under logs/gcg_complete/base-natural, matching
prefix_run_worker.py's LOG_DIR in this same folder.
"""
...
LOG_DIR = 'logs/gcg_complete/base-natural'
N_PROCESSES = 5
K_INSERTIONS = 5
VARIANTS = ["prefix"]
...
SUMMARY_PATHS = [
    os.path.join(LOG_DIR, f"insertion_accelerated_refinement_summary_p{i}.csv")
    for i in range(N_PROCESSES)
]
ALL_SUMMARY_PATH = os.path.join(LOG_DIR, "insertion_accelerated_refinement_summary_all.csv")
PLOT_PATH = os.path.join(LOG_DIR, "insertion_accelerated_refinement_loss_curves.png")
STATS_PATH = os.path.join(LOG_DIR, "insertion_accelerated_refinement_aggregate.txt")
...
def print_aggregate_stats(df, out_file=None):
    lines = []
    lines.append(f"{'='*70}")
    lines.append(f"AGGREGATE RESULTS -- {df['prompt_idx'].nunique()} unique prompts, "
                 f"{len(df)} total rows")
    lines.append(f"{'='*70}")
    ...
    for variant in VARIANTS:
        for acceptance_mode in ACCEPTANCE_MODES:
            ...
            lines.append(f"  mean cost change:         {sub['cost_pct_change'].mean():+.1f}%")
            lines.append(f"  median cost change:       {sub['cost_pct_change'].median():+.1f}%")
            ...
            # Best-ever-seen point (identical to the walk endpoint under
            # "strict"; only informative for "unconditional").
            if "best_natural_cost" in sub.columns:
                lines.append(f"  -- best point observed at any iteration --")
                lines.append(f"  mean best cost change:     {sub['best_cost_pct_change'].mean():+.1f}%")
                ...
                # Drift between the walk's endpoint and its own best point
                # -- should be ~0 for "strict" (sanity check), and is the
                # direct jitter-cost measure for "unconditional".
                drift = sub["final_natural_cost"] - sub["best_natural_cost"]
...
def plot_loss_curves(df):
    ...
    # One panel per acceptance_mode -- "strict" (monotonic) and
    # "unconditional" (jittery) traces are structurally different, and
    # each prompt appears once per mode, so overlaying would be misleading.
    ...
    for ax, acceptance_mode in zip(axes, modes_present):
        ...
        for i, (_, row) in enumerate(sub.iterrows()):
            ...
            # Split insertion phase from refinement phase
            insertion_part = trace[:K_INSERTIONS + 1]
            refine_part = trace[K_INSERTIONS + 1:]

            # Running minimum over the insertion phase (each insertion
            # commits the best filler, so this phase is non-increasing
            # regardless of acceptance_mode -- that setting only governs
            # the refinement phase below).
            running_min, best = [], float("inf")
            for c in insertion_part:
                best = min(best, c)
                running_min.append(best)

            # refine_part is left raw (no smoothing): monotonic under
            # "strict" by construction, but "unconditional"'s jitter is
            # the point of that mode and must not be smoothed away.
        ...
        ax.set_ylabel("natural_order_cost")
        ax.set_title(f"acceptance_mode = {acceptance_mode}  (n={len(sub)})")
        ax.legend(fontsize=6, ncol=4, loc="upper right")
        ax.grid(True, alpha=0.3)

    fig.suptitle(
        f"Loss curves -- prefix insertion attack, {sub_all['prompt_idx'].nunique()} prompts\n"
